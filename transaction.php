...
$userID=$_SESSION['userID'];
	...
	if (isset($_SESSION['userRole']))
	{
		...
		{
			$result = mysql_query("select  t.transactionID,tp.status_payment,tp.paymentID,tp.nama_payment,tp.nama_bank,tp.nilai_transfer,tp.norek, ts.shippingID,ts.nama_shipping,ts.status_shipping, t.transactionID, t.date , t.status_transaction, d.productID, d.quantity,d.S,d.M,d.L,d.XL,d.XXL,d.XXXL,p.image,p.productname,p.price from trtransaction as t,detailtransaction as d , msproduct as p,trshipping AS ts, trpayment as tp
			where t.transactionID=d.transactionID and d.productID = p.productID and t.userID=$userID
...
AND t.transactionID = tp.transactionID");
			...
			$tanda=0;
			$grand=0;
			$list.="<h1>Status Regular List</h1>
						<div class='in-check' >
				<ul class='unit'>
			...
			while($row = mysql_fetch_array($result))
			{
				...
				$list.="<ul class='cart-header'>
					 <li align='left' class='ring-in'><p>$username</p>
				 <li align='left'>
...
");
			...
			$elist.=' <h1>Status Custom List</h1>
						<div class="in-check" >
				<ul class="unit">
				   <li><span>Nama</span></li>
				   <li><span>Product</span></li>
		...
		{$userID=$_SESSION['userID'];
		$result = mysql_query("SELECT tt.transactionID,tp.status_payment,tp.paymentID,tp.nama_payment,tp.nama_bank,tp.nilai_transfer,tp.norek, ts.shippingID,ts.nama_shipping,ts.status_shipping,tt.status_transaction, tt.date, tt.status_transaction,dc.status_customize, dc.customname, dc.harga, dc.bahan, dc.warna, dc.image, dc.keterangan, dc.customizeID, dc.quantity,dc.S, dc.M, dc.L, dc.XL,dc.XXL, dc.XXXL
FROM trtransaction AS tt, detailcustomize AS dc, trshipping AS ts, trpayment as tp
